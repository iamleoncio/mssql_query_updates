...
CASE WHEN IsNull(l.BalPrin+l.balint,0) <= 0 THEN 1 ELSE           ---modified by fritz                             
  ...
  CASE cd.Code                                         
   ...
   CASE cd.Code                                         
     WHEN 0 THEN 0                                        
     WHEN 1 THEN 0                                        
     WHEN 3 THEN l.BalPrin+l.iBalInt+l.iBalOth - WaivedInt ELSE 0 END LoanBal,                                        
   CASE cd.Code                                         
     WHEN 0 THEN 0                                        
     WHEN 1 THEN s.SaveBal                                        
...
LEFT JOIN                                          
 (SELECT                                         
    ...
    SUM(CASE     
             ...
             WHEN DBO.Friday(DueDate) > DBO.Friday(ebSysDate) --AND IsNull(WaivableInt,1) = 1    
              and duedate<= DOMATURITY + 6 - Datepart(dw,domaturity)     and m.accttype not in (420,461,475,323,321,483)                                    
