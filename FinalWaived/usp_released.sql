...
BEGIN                   
    ...
    BEGIN                  
        ...
        CHRBAL = CEILING((@pprincipal / 1000.00) * 0.3 * @pterm) 
    ...
    BEGIN                  
        ...
        CHRBAL = CEILING((0.015 / 12 * @pterm) * @pprincipal)   
        WHERE ACC = @Acc AND CHRGCODE = 16;                  
    END                  
    ...
    BEGIN                  
        ...
        SET CHRAMNT = CEILING((0.015 / 24 * @pterm) * @pprincipal) ,
         CHRBAL = CEILING((0.015 / 24 * @pterm) * @pprincipal)
        WHERE ACC = @Acc AND CHRGCODE = 16;                  
    END                      
    IF (                  
...
UPDATE LOANINST set origDueDt = ori.OrigDueDt from dbo.fn_getOrigDuedateAcc(@Acc) ori  where ori.acc = loaninst.acc and ori.dnum = loaninst.dnum
-------------------------------------------------------------------------------------------------                                      
