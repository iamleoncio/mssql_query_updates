...
IF NOT EXISTS (SELECT 1 FROM refmapping WHERE gabaykonek = @pbusinesstype) OR @pbusinesstype = ''                                
    SET @pbtype = 1588                                
ELSE                                
    SELECT @pbtype = ISNULL(localcode, 1588)                                
    FROM refmapping                                
    WHERE gabaykonek = @pbusinesstype                                
...
------------------------- CREATE LOAN DETAILS -----------------                                             
 ...
 SET @pInterest =                                                               
  ...
  EXEC usp_CalculateCharges @pType, @pFrequency, @pPrincipal, @pTerms,@pAge,@pSubclass, @pCharges OUTPUT;                       
    ...
    SELECT *,NULL FROM dbo.genloaninstv2(@pAcc, @pPrincipal, @pInterest, @pTerms, @pEffrate, @pDateRel);   
...
DECLARE @tAmortAcc varchar(22)                                                          
   SELECT TOP 1   @tAmortAcc = lo.acc                                                           
    ...
    END;                                                          
    UPDATE LOANINST set origDueDt = ori.OrigDueDt from dbo.fn_getOrigDuedateAcc(@pAcc) ori  where ori.acc = loaninst.acc and ori.dnum = loaninst.dnum                                                          
