...
------------------------- CREATE LOAN DETAILS -----------------                                                   
 ...
 SET @pInterest =                                                                     
  ...
  EXEC usp_CalculateCharges @pType, @pFrequency, @pPrincipal, @pTerms,@pAge,@pSubclass, @pCharges OUTPUT;                             
    ...
    SELECT DNUM,ACC,DUEDATE,InstFlag, PRIN , INTR,OTH, PENALTY,ENDBALPRIN, ENDBALINT,ENDOTH,INSTPD,PENPD,CARVAL,UPINT,SERVFEE,PLEDGEAMORT,AMORT_OTH,BALANCE_OTH  FROM dbo.fn_amortization(@pAcc, @pPrincipal, @pTerms, @pFrequency,@pType, @pDateRel, @pInterest);                                    
...
------------------------- COMPUTE LRF -----------------                                                                    
  ...
  DECLARE @pWaived integer;  
                                                                 
SELECT @pCharges2 = (PRINCIPAL - prin) + (INTEREST - intr) , @pprevACC = acc                                                                    
FROM lnmaster                                                                    
 WHERE cid = @pCID AND ACCTTYPE = @pType AND status IN (30, 91);   
   
 select @pWaived = waivedint from CurrentLoanBal(@pprevacc)  
  
                                                                    
        INSERT INTO LNCHRGDATA (acc, chd, CHRGCODE, CHRDESC, CHRAMNT, CHRBAL, RefAcc)                                                                    
        VALUES (@pAcc, 0, 18, 'Previous Loan', @pCharges2 - @pWaived, @pCharges2 -  @pWaived, @pprevACC);                                                                    
