 ...
 */    
 ...
 ALTER PROCEDURE usp_Released(@Acc         as VarChar(20),                                               
...
@pPrincipal = principal,          
    ...
    begin 
    update LNCHRGDATA SET CHRAMNT = CHRBAL WHERE ACC = @Acc AND CHRGCODE = 16; 
    end
    IF (          
...
-- ------------------ GLIP Control ------------------
IF @AcctType IN (301, 316, 461, 450, 449)
BEGIN
    DECLARE 
        @pdobirth DATE,
        @pseventy DATE,
    ...
    SELECT 
        @pdobirth = c.DOBIRTH,
        @pdorecognized = c.DORECOGNIZED,
        @ebsysdate = o.ebsysdate
    FROM CUSTOMER c
    CROSS JOIN ORGPARMS o
    WHERE c.CID = @CID;
    SELECT 
    ...
    SET @actualAge = 
        CASE 
            WHEN MONTH(@ebsysdate) > MONTH(@pdobirth) 
                 OR (MONTH(@ebsysdate) = MONTH(@pdobirth) AND DAY(@ebsysdate) >= DAY(@pdobirth)) 
            THEN DATEDIFF(YEAR, @pdobirth, @ebsysdate)
            ELSE DATEDIFF(YEAR, @pdobirth, @ebsysdate) - 1
        END;
    IF (
        @pdorecognized > '2006-06-01' AND 
        @pdomaturity > @pseventy AND 
        @actualAge < 70
    )
    BEGIN
    ...
    IF (
        @pdorecognized > '2006-06-01' AND 
        @pdomaturity > @pseventypayb AND 
        @actualAge BETWEEN 70 AND 74
    )
    BEGIN
